...
indexTurnover= GetIndexFromChoose("turnover")
plt.figure('Average')
for iIndexTurnover in indexTurnover:
    #iIndexTurnover='603991'
    openCode=ReadHistDataIndex(iIndexTurnover,'open')

    openCodeSmooth=Smooth(openCode, 15,5)
    
    plt.clf()
    plt.hold('on')
    plt.plot(openCode,'b')
    plt.plot(openCodeSmooth,'r')
    plt.pause(0.1)
    
#yhat = savitzky_golay(y, 51, 3)

print len(indexTurnover)
    
'''
kindChoose='Openlast'
figureDir='Openlast'
contentDataCode='open'
colorPlot='b'
PlotInforD1(kindChoose,figureDir,contentDataCode,colorPlot)
'''
