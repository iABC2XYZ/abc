...
ioput_dim=1                   # Output dimension (e.g.: multiple signals at once, tied in time)
...
def generate_x_y_data(indexCode,isTrain, seq_length,batch_size,ioput_dim):
    ...
    nData=len(openCode)
    NChoose=nData-nSeq
    nChoose= np.random.randint(low=0,high=NChoose+1,size=batch_size*ioput_dim)
...
indexCode='603505'
batch_x,batch_y=generate_x_y_data(indexCode,isTrain=True, seq_length=seq_length,batch_size=batch_size,ioput_dim=ioput_dim)
...
for _ in xrange(batch_size):
    for nIO in xrange(ioput_dim):
        plt.plot(np.arange(inSeq),batch_x[:,_,nIO])
        plt.hold('on')
        plt.plot(np.arange(outSeq)+inSeq,batch_y[:,_,nIO])
...
def TensorFlowPredict(indexCode,inSeq,outSeq,batch_size,hidden_dim,ioput_dim,nb_iters):
    ...
    momentum = 0.00  # default: 0.0 . Momentum technique in weights update
    ...
    for t in range(nb_iters + 1):
        ...
        X, Y = generate_x_y_data(indexCode,isTrain=True, seq_length=seq_length,batch_size=batch_size,ioput_dim=ioput_dim)
    ...
    ######################PREDICT:
    #X=Generate_x_data4Future(indexCode,isTrain=True, seq_length=seq_length,batch_size=batch_size,ioput_dim=ioput_dim)
    ...
    X4FutureTmp=np.empty((inSeq,batch_size,ioput_dim))
    for iB in xrange(batch_size):
        for iIO in xrange(ioput_dim):
    ...
    #plt.plot(np.arange(inSeq)+nData,Y4Future,'ro')
...
indexCode='300401'
TensorFlowPredict(indexCode,inSeq,outSeq,batch_size,hidden_dim,ioput_dim,nb_iters)
