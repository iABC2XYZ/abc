...
for iLatticeM=1:LatticeM
    ...
    save('Check.txt','M','-append','-ascii')
    save('Check.txt','Blank','-append','-ascii')
    
    figure(1)
    clf;
    
    subplot(2,2,1)
    plot(partDistri(:,1),partDistri(:,2),'b.');
    hold on
    plot(partDistri(flagPart==1,1),partDistri(flagPart==1,2),'r.'); 
    
    subplot(2,2,2)
    plot(partDistri(:,3),partDistri(:,4),'b.');
    hold on
    plot(partDistri(flagPart==1,3),partDistri(flagPart==1,4),'r.');     
    
    subplot(2,2,3)
    plot(partDistri(:,1),partDistri(:,3),'b.');
    hold on
    plot(partDistri(flagPart==1,1),partDistri(flagPart==1,3),'r.');   
     subplot(2,2,4)
    title(iLatticeM)
     pause(0.1)
end
