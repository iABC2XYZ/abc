...
def CheckACCT2(iCut=1.):    #--------------------
    iACCT2=caget('ADS_SLIT_VALUE.CHAN5_VAL_M')
    acct2False=0
    if iACCT2<iCut:
        acct2False=1
    return iACCT2,acct2False
...
def ReadWrite_ALL(fid,bpmList,psSetList,psMonList,mainMonList,psSetListLib,psMonListLib,mainMonListLib):
    i3Cut=0.3
    i2Cut=1.
    iACCT3,flagACCT3= CheckACCT3(i3Cut)
    iACCT2,flagACCT2= CheckACCT2(i2Cut)
    ...
    LogWrite(fid,(bpmNow4W,psSetNow4W,psMonNow4W,psMainSet4W,iACCT2,iACCT3))
...
while True:
    ...
    psMainSet,psMainMon,idFalse=Get_Check_Main_PS(mainMonList)
    i3Cut=0.2
    iACCT3,acct3False= CheckACCT3(i3Cut)
    if len(idFalse)>0:
        for i in idFalse:
            print mainMonList[i]
            flagPut=0
    ...
    if acct3False==1:
        print 'ACCT3'
        flagInput=raw_input('Run: 1        ;  Continue: 0  ')
        if flagInput[0]=='1':
            flagPut=1
        if flagInput[0]=='0':
            flagPut=0
