...
for _ in range(1000):
    x=np.random.rand((numPart))*2-1.+np.random.randn(numPart)*0.5
    xp=np.random.rand((numPart))*2-1.+np.random.randn(numPart)*0.3
    covNoise=np.array([[5.,1.,],[1.,0.4]])
    meanNoise=np.array([0.,0.])
    xNoise,xpNoise=np.random.multivariate_normal(meanNoise,covNoise,numPart).T
    x=np.add(x,xNoise/2.)
    xp=np.add(xp,xpNoise/2.)
